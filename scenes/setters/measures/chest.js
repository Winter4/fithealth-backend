...
const composeWizardScene = require('../require-belt').factory.composeWizardScene;
...
const setChestMeasureScene = composeWizardScene(
    ctx => {
        ctx.reply(`Введите длину окружности груди (${limits.min}-${limits.max} см):`, Markup.removeKeyboard());
    ...
    (ctx, done) => {
        if (ctx.message.text) {
            ...
            ctx.session.user.measures.chest = length;
            return done();
        }
        else {
...
module.exports.scene = setChestMeasureScene;