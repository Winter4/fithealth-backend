...
const setWaistMeasure = require("./waist.measure.setter");
const mainMenu = require("../../menus/main/home.menu.main");

const SCENE_ID = "SET_CHEST_MEASURE";
...
async function enter(ctx) {
  try {
    ...
    return ctx.replyWithPhoto(
      { source: photoSource },
      {
        caption: `Введите обхват груди (${Limits.min}-${Limits.max} см):`,
        keyboard: Markup.removeKeyboard(),
      }
    );
  } catch (e) {
    throw new Error(
      `Error in <enter> middleware of <chest.measure.setter> scene --> ${e.message}`
    );
  }
}
...
scene.on(
  ...
  async (ctx, next) => {
    try {
      ...
      throw new Error(
        `Error in validation of <on_text> middleware of <chest.measure.setter> scene --> ${e.message}`
      );
    }
  },
  ...
  async (ctx, next) => {
    try {
      // update data
      await User.set.measure.chest(ctx.chat.id, ctx.message.text);

      return next();
    } catch (e) {
      throw new Error(
        `Error in data_update of <on_text> middleware of <chest.measure.setter> scene --> ${e.message}`
      );
    }
  },
  ...
  async (ctx) => {
    try {
      ...
      throw new Error(
        `Error in scene_push of <on_text> middleware of <chest.measure.setter> scene --> ${e.message}`
      );
    }
  }
);
