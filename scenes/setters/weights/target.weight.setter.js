...
const SCENE_ID = "SET_TARGET_WEIGHT";
...
async function enter(ctx) {
  try {
    ...
    return ctx.replyWithHTML(
      `Введите свой <b><i>желаемый</i></b> вес числом (${Limits.min}-${Limits.max} кг):`,
      Markup.removeKeyboard()
    );
  } catch (e) {
    throw new Error(
      `Error in <enter> middleware of <target.weight.setter> scene --> ${e.message}`
    );
  }
}
...
scene.on(
  ...
  async (ctx, next) => {
    try {
      ...
      throw new Error(
        `Error in validation of <on_text> middleware of <target.weight.setter> scene --> ${e.message}`
      );
    }
  },
  ...
  async (ctx, next) => {
    try {
      // update data
      await User.set.weight.target(ctx.chat.id, ctx.message.text);

      return next();
    } catch (e) {
      throw new Error(
        `Error in data_update of <on_text> middleware of <target.weight.setter> scene --> ${e.message}`
      );
    }
  },
  ...
  async (ctx) => {
    try {
      ...
      const enterNextScene = (await User.get.registered(ctx.chat.id))
        ? mainMenu.enter
        : setChestMeasure.enter;

      // push to next scene
      return enterNextScene(ctx);
    } catch (e) {
      throw new Error(
        `Error in scene_push of <on_text> middleware of <target.weight.setter> scene --> ${e.message}`
      );
    }
  }
);
...
scene.on("message", (ctx) => {
  return ctx.reply("Пожалуйста, введите желаемый вес числом");
});
